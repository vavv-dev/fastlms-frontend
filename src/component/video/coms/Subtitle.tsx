import {
  VideoGetSubtitlesData,
  VideoGetSubtitlesResponse,
  VideoGetViewData,
...
import { Box, Checkbox, FormControlLabel, IconButton, InputAdornment, Stack, Switch, TextField } from '@mui/material';
...
const Subtitle = ({ videoId }: { videoId: string }) => {
  ...
  const { data: subtitles } = useServiceImmutable<VideoGetSubtitlesData, VideoGetSubtitlesResponse>(videoGetSubtitles, {
    id: videoId,
  });
...
interface ISubtitleLines {
  tracksRef: React.MutableRefObject<Record<number, Record<string, string>>>;
  langs: Record<string, boolean>;
}

// use memo
const SubtitleLines = React.memo(({ tracksRef, langs }: ISubtitleLines) => {
  const { t } = useTranslation('video');
  const playerInstance = useAtomValue(playerInstanceState);
