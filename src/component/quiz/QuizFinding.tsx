import { QuizAssessResponse, QuizGetAssessData, quizGetAssess } from '@/api';
...
const QuizFinding = ({ quizId }: { quizId: string }) => {
  const { t } = useTranslation('quiz');
  const theme = useTheme();
  const { data: quiz } = useServiceImmutable<QuizGetAssessData, QuizAssessResponse>(quizGetAssess, { id: quizId });

  if (!quiz || !quiz.finding) return null;
  ...
  return (
    <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2, width: '100%', '& .percentage': { minWidth: 60 } }}>
      ...
      {Object.entries(quiz.finding).map(([id, _question], i) => {
        ...
        const answer = Number(quiz.submission?.answers?.[id]);
