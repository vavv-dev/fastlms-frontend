...
const ReportDialog = ({ open, setOpen, lesson }: IProps) => {
  ...
  const downlaodXlsxFile = async () => {
    if (!lesson) return;
    const text = await lessonDownloadLessonReport({
      id: lesson.id,
      asOf: String(new Date(asOf).getTime() || '') || null,
      upTo: String(new Date(upTo).getTime() || '') || null,
    });
    const filename = `${lesson.title}.${asOf || 'all'}~${upTo || new Date().toISOString().slice(0, 16)}.xlsx`;
  ...
  return (
    <BaseDialog
      ...
      renderContent={() => (
        <GridInfiniteScrollPage<LessonReportResponse, LessonGetLessonReportData>
          pageKey="answerlist"
          apiService={lessonGetLessonReport}
          apiOptions={{
            id: lesson.id,
                ...
                <Table size="small" sx={{ tableLayout: 'fixed', '& *': { whiteSpace: 'noWrap' }, '& td': { px: 1 } }}>
                  ...
                  <TableBody>
                    {data?.map((pagination, i) =>
                      pagination.items?.map((watch: LessonReportResponse) => (
                        <TableRow key={i}>
                          <TableCell>
