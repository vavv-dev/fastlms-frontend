...
import { InvitationAcceptRequest, publicInvitationAccept } from '@/api';
import { Form, TextFieldControl as Text } from '@/component/common';
import { alertState } from '@/component/layout';
import { userState } from '@/store';

interface InvitationAcceptRequest2 extends InvitationAcceptRequest {
...
export const InvitationAccept = () => {
  ...
  const { handleSubmit, control, setError, formState } = useForm<InvitationAcceptRequest>({
    resolver: yupResolver(schema),
    defaultValues: { ...schema.getDefault(), token: searchParams.get('token') || '' },
  });

  const acceptInvitation = (data: InvitationAcceptRequest2) => {
  ...
  return (
    <Container component="main" maxWidth="xs">
      <Box sx={{ mt: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', '& form': { width: '100%' } }}>
        ...
        <Form onSubmit={handleSubmit(acceptInvitation)} formState={formState} setError={setError}>
