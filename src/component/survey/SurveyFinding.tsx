import { SurveyAssessResponse, SurveyGetAssessData, surveyGetAssess } from '@/api';
...
const SurveyFinding = ({ surveyId }: { surveyId: string }) => {
  const { t } = useTranslation('survey');
  const theme = useTheme();
  const { data: survey } = useServiceImmutable<SurveyGetAssessData, SurveyAssessResponse>(surveyGetAssess, { id: surveyId });

  if (!survey || !survey.finding) return null;
  ...
  return (
    ...
    >
      {Object.entries(survey.finding).map(([id, { kind, question, mandatory, occurrences, selections }], i) => {
